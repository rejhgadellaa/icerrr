...
copy %cd%\..\_apks\%name_prj%-release-test.apk D:\Desktop\Dropbox\__Static\icerrr\tmp_apks\%name_prj%-release-test.apk
if errorlevel 1 goto error
if not errorlevel 0 goto error

echo.
echo Install and run on device...
%androidsdk%platform-tools\adb -d install -r D:\Desktop\Dropbox\__Static\icerrr\tmp_apks\%name_prj%-release-test.apk
if errorlevel 1 goto error
if not errorlevel 0 goto error
